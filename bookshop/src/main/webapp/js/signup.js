...
window.addEventListener("load", () => {
    document.querySelectorAll(".pwd").forEach(item => {
        const input = item.querySelector("input[name='password']");
        const button = item.querySelector(".view");

        button.addEventListener("mausedown", () => {
            input.setAttribute("type", "text");
        });

        button.addEventListener("mauseup", () => {
    ...
    document.getElementById("signup").addEventListener("click", e => {
        ...
        if(form.password.value !== form.check_password.value){
            alert("비밀번호와 비밀번호 확인이 일치하도록를 입력하셔야 합니다.");
